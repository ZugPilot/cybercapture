...
public class ComputerUI extends UI {
    ...
    public ComputerUI(CyberGame cyberGame, Computer computer) {
        ...
        this.generationUI = new GenerationUI(cyberGame, this);
    }
    ...
    public void build() {
        ...
        addElement(4, new UIElement(new ItemBuilder(Material.GRAY_DYE).displayName("§7Computer-Stats:").setLore(
                "§7Leben: " + computer.getHealth(),
                "§7Points: " + computer.getTeam().getPoints(),
                "§7Antiviren Software: " + (computer.getProtectingProcess().isPresent() ? "Nicht gefunden" : computer.getProtectingProcess().get().getColoredProcessName()),
                "§7CPU: " + (computer.getGeneratingProcess().isPresent() ? "Nicht gefunden" : computer.getGeneratingProcess().get().getColoredProcessName())).build()));
